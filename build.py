...
"""Сборка страниц из частей.
   ...
   Версия ассетов подставляется в {{v}} из `_build.json`; ссылкам на картинки
   `assets/img/` сборка дописывает её сама.
...
def build(src, v):
    ...
    # Картинкам версия дописывается сама: заменили файл под тем же именем —
    # браузер не отдаст старый из кеша. Руками `?v=` у картинок не ставим.
    out = re.sub(r'(assets/img/[^"\s,?]+\.(?:jpe?g|png|webp|svg|gif|avif))(?=["\s,])',
                 lambda m: f'{m.group(1)}?v={v}', out)
    # Комментарии из собранного вырезаются: в исходниках это заметки для
    # разработчика и для себя (решения, цифры аудита, вопросы менеджеру),
    # а собранная страница публикуется. Строка из одного комментария
    # уходит целиком, чтобы не оставались пустые строки.
    out = re.sub(r'^[ \t]*<!--(?:(?!-->).)*-->[ \t]*\n', '', out, flags=re.M | re.S)
